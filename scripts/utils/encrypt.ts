...
export function processBlogItem (
  ...
  const processJson = async (type: HeaderTabUrl) => {
    ...
    const itemList: CommonItem[] = JSON.parse(
    ...
    for (const item of itemList) {
      ...
      if (item.encrypt) {
        ...
        for (const block of item.encryptBlocks) {
          ...
          decryptedMd =
            decryptedMd.slice(0, start) +
            (await _decrypt(decryptedMd.slice(start, end))) +
            decryptedMd.slice(end);
        }
      }
    ...
    console.log(
      colors.bold.bgCyan(
        type.substring(1) + ` (${count} in total) processing completed!`
      )
    );
  };
 ...
 */
export async function encryptAndWriteMd ({
  item,
  md,
  path,
  encrypt,
