        ...
        for (const block of item.encryptBlocks!) {
          ...
          newMarkdownContent = logined
            ? // 如果已登录：给block显示为sticker表情
              ...
              translate('encrypted-content') +
              '![sticker](aru/59)' +
              newMarkdownContent.slice(end)
            : // 如果未登录：直接隐藏block
              newMarkdownContent.slice(0, start - 10) +
              newMarkdownContent.slice(end + 11)
        }
...
function writeDate(time: number, modifyTime: number) {
  ...
  return defineComponent({
    render: () => (
      <div
        class="write-date flex"
        onClick={() => (fuzzyMode.value = !fuzzyMode.value)}
      >
        <SvgIcon name="write" />
        <div class="flexc">
          {fuzzyMode.value && <span>{literalTime(time)}</span>}
          {!fuzzyMode.value && (
            <>
              <span>
                {translate('created-at')}: {formatTime(time)}
              </span>
              <span>
                {translate('updated-at')}: {formatTime(modifyTime)}
              </span>
            </>
          )}
        </div>
